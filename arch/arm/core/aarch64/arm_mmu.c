...
#define XLAT_TABLE_VA_IDX(va_addr, level) \
	((va_addr >> LEVEL_TO_VA_SIZE_SHIFT(level)) & 0x1ffULL)

/*
 * Calculate the initial translation table level from CONGIG_ARM64_VA_BITS
 * For a 4 KB page size,
 * (va_bits <= 21)	 - base level 3
 * (22 <= va_bits <= 30) - base level 2
 * (31 <= va_bits <= 39) - base level 1
 * (40 <= va_bits <= 47) - base level 0
 */
...
static u64_t get_tcr(int el)
{
	...
	if (el == 1) {
		tcr = TCR_EL1_RSVD | (tcr_ps_bits << TCR_EL1_IPS_SHIFT);
		/*
		...
		tcr = TCR_EL3_RSVD | (tcr_ps_bits << TCR_EL3_PS_SHIFT);
